...
-export([
    ...
    rdfs_namespace_iri/0,
    rdfs_namespaced_iri/1
]).

% Namespace IRI for 'xsd'/'http://www.w3.org/2001/XMLSchema'.
...
% Namespace IRI for 'rdf'/'http://www.w3.org/1999/02/22-rdf-syntax-ns'.
-spec rdf_namespace_iri() -> iri().
rdf_namespace_iri() ->
...
rdf_namespaced_iri(TermName) when is_binary(TermName) ->
    <<(rdf_namespace_iri())/binary, TermName/binary>>.


% Namespace IRI for 'rdfs'/'http://www.w3.org/2000/01/rdf-schema'.
-spec rdfs_namespace_iri() -> iri().
rdfs_namespace_iri() ->
...
rdfs_namespaced_iri(TermName) when is_binary(TermName) ->
    <<(rdfs_namespace_iri())/binary, TermName/binary>>.
